...
| Metric | Value |
|--------|-------|
| Papers catalogued | ~31,650 |
| PDF files on disk | ~20,400 |
| Extraction columns | ~1,740 total; 127 binary across the 6 core schemas |
| Evidence rows (audit trail) | ~293,000 across ~19,900 papers |
| Unique authors (OpenAlex) | ~29,900 |
| Species columns | ~1,300 |
| Techniques in taxonomy | 215 |
| Conference abstracts (grey literature) | ~9,800, of which ~3,200 chondrichthyan |

*Rounded, and growing with each monthly sync. Exact current figures are generated by `python3 scripts/report_corpus_stats.py` into `outputs/corpus_stats.json`; the full as-built pipeline and the canonical table live in the [pipeline overview](docs/core/pipeline_overview.md).*
...
**3. Extract.** `scripts/extract_schema_columns.py` reads each PDF's text and scores it against **127 binary columns across the 6 core schemas** (discipline, ecosystem, pressure, gear, impact, ocean basin), plus 43 sub-basin, 1,308 species, and 215 analytical-technique columns, using keyword-plus-context matching with section-weighted scoring. Every classification is backed by a quote in the evidence table. See [extraction logic](docs/schema_proposals/extraction_logic.md).
...
Six schema categories extract binary classifications from each PDF. Fully implemented and run across the corpus. Column counts below are as built in the parquet, not as originally proposed.

| Category | Prefix | Columns | Proposal |
|----------|--------|---------|----------|
| Discipline | `d_` | 19 | [discipline_proposal.md](docs/schema_proposals/discipline_proposal.md) |
| Ecosystem | `eco_` | 23 | [ecosystem_component_proposal.md](docs/schema_proposals/ecosystem_component_proposal.md) |
| Pressure / Threat | `pr_` | 26 | [pressure_proposal.md](docs/schema_proposals/pressure_proposal.md) |
| Fishing Gear | `gear_` | 29 | [gear_proposal.md](docs/schema_proposals/gear_proposal.md) |
| Impact / Response | `imp_` | 25 (21 binary + 4 metadata) | [impact_proposal.md](docs/schema_proposals/impact_proposal.md) |
| Ocean Basin | `b_` | 9 | [ocean_basin_proposal.md](docs/schema_proposals/ocean_basin_proposal.md) |

Beyond these six: `sb_` sub-basin (43), `sp_` species (1,308), `a_` analytical technique (215), `geo_` geography (19), `ob_` ocean basin from the geographic pipeline (9), `depth_` (3).
...
- **Database query interface** — natural-language querying of the corpus, running over the full index with schema pre-filters. **Its retrieval quality has not yet been measured**; that work is scoped in [the retrieval evaluation design](docs/superpowers/specs/2026-08-06-retrieval-evaluation-design.md) and should land before the interface is opened to external users.
- **Next** — methods manuscript and public database release.

Known open gaps are listed in the [pipeline overview](docs/core/pipeline_overview.md#open-gaps-scheduled-work-not-known-unknowns).

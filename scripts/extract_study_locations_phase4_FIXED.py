#!/usr/bin/env python3
"""
Phase 4 FIXED: Extract Study Locations from Methods Sections

FIXES:
1. Word boundary matching (prevent "incubated" matching "cuba")
2. Corroboration requirement (country must appear near study-related keywords)
3. Context validation (look for "study site", "sampling location", etc.)
...
# Study location keywords (for corroboration)
...
# NOTE: Multi-word patterns will be checked first (longest match priority)
COUNTRIES = {
...
def extract_country_with_context(text: str) -> Optional[str]:
    """
    Extract country name from text using WORD BOUNDARY matching and CORROBORATION.

    Returns country only if:
    1. Country name appears as a complete word (not substring)
    2. Country appears near study location keywords (within 100 chars)
    """
    if not text:
        return None
    ...
    for country, pattern in all_patterns:
        ...
        if matches:
            # Check if ANY match is near a study location keyword
...
def extract_study_location_snippet(text: str) -> Optional[str]:
    ...
    for sentence in sentences[:50]:  # First 50 sentences
        ...
        if any(kw in sentence_lower for kw in STUDY_LOCATION_KEYWORDS):
            # Check if it mentions a country or ocean
            if extract_country_with_context(sentence) or extract_ocean_basin(sentence):
...
def process_paper(paper_info: Tuple[str, str, str]) -> Dict:
    ...
    # Extract geographic information WITH CONTEXT VALIDATION
    study_country = extract_country_with_context(methods_text)
...
def main():
    ...
    print("\n" + "="*80)
    print("PHASE 4 FIXED: EXTRACT STUDY LOCATIONS FROM METHODS SECTIONS")
    print("="*80 + "\n")

    print("FIXES APPLIED:")
    print("  1. Word boundary matching (prevents 'incubated' matching 'cuba')")
    print("  2. Context validation (country must appear near study keywords)")
    print("  3. Longest pattern first (prevents 'Wales' before 'New South Wales')")
    print()

    print("Goal: Identify WHERE research was conducted vs WHERE authors are based")
    print("Enables: 'Parachute research' analysis (author country ≠ study country)\n")

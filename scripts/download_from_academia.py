#!/usr/bin/env python3
"""
download_from_academia.py

Download papers from Academia.edu using authenticated access.

⚠️  EDUCATIONAL/RESEARCH USE ONLY
This script automates Academia.edu access. Use responsibly and ensure
compliance with Academia.edu's Terms of Service.
...
# ============================================================================
# ACADEMIA.EDU FUNCTIONS
# ============================================================================

class AcademiaSession:
    """Handle Academia.edu authentication and requests."""
    ...
    def login(self):
        ...
        print("\n🔐 Logging in to Academia.edu...")

        try:
            # Get login page to retrieve CSRF token
            login_page_url = "https://www.academia.edu/login"
            ...
            # Academia typically redirects to main page on success
            if response.status_code == 200 and ('login' not in response.url or 'dashboard' in response.url):
                print("✅ Successfully logged in to Academia.edu")
                self.logged_in = True
                return True
            else:
    ...
    def search_paper(self, title=None, authors=None):
        """
        Search for a paper on Academia.edu.
        ...
        try:
            ...
            # Search Academia.edu
            search_url = f"https://www.academia.edu/search?q={requests.utils.quote(query)}"
            ...
            # Academia uses various patterns for paper links
            paper_links = soup.find_all('a', href=re.compile(r'/\d+/'))

            if paper_links and len(paper_links) > 0:
                # Get first result
                paper_path = paper_links[0].get('href')
                if not paper_path.startswith('http'):
                    paper_url = f"https://www.academia.edu{paper_path}"
                else:
    ...
    def get_pdf_url(self, paper_url):
        ...
        Args:
            paper_url: URL of paper on Academia.edu
        ...
        try:
            ...
            # Academia uses various patterns for PDF links
            pdf_patterns = [
                ('a', {'class': re.compile(r'.*download.*', re.I)}),
                ('a', {'href': re.compile(r'.*\.pdf$')}),
                ('button', {'data-download-url': re.compile(r'.*')})
            ]

            for tag, attrs in pdf_patterns:
                pdf_link = soup.find(tag, attrs)
                if pdf_link:
                    href = pdf_link.get('href') or pdf_link.get('data-download-url')
                    if href:
                        if not href.startswith('http'):
                            href = f"https://www.academia.edu{href}"
                        return href
    ...
    def download_pdf(self, pdf_url, output_path):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if 'application/pdf' in content_type or pdf_url.endswith('.pdf'):
                    ...
                    with open(output_path, 'rb') as f:
                        ...
                        if header == b'%PDF':
                            return {
                                'status': 'success',
                                'message': 'Downloaded from Academia.edu',
                                'file_size': file_size
                            }
                        else:
                            output_path.unlink()
                ...
                return {
                    'status': 'forbidden',
                    'message': 'Access forbidden - may require premium access',
                    'file_size': 0
                }
            else:
                return {
...
def main():
    parser = argparse.ArgumentParser(description="Download papers from Academia.edu")
    ...
    print("\n" + "=" * 80)
    print("ACADEMIA.EDU DOWNLOADER")
    print("=" * 80)
    print("⚠️  EDUCATIONAL/RESEARCH USE ONLY")
    print("⚠️  Ensure compliance with Academia.edu Terms of Service")
    print("=" * 80)
    print(f"Started: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

    # Initialize Academia session
    academia = AcademiaSession(ACADEMIA_EMAIL, ACADEMIA_PASSWORD)

    if not academia.login():
    ...
    for idx, row in tqdm(needs_pdf.iterrows(), total=len(needs_pdf), desc="Downloading"):
        ...
        # Search for paper
        search_result = academia.search_paper(title=title, authors=authors)

        if search_result and search_result.get('found'):
            paper_url = search_result['paper_url']

            # Get PDF URL
            pdf_url = academia.get_pdf_url(paper_url)
            ...
            if pdf_url:
                ...
                # Download PDF
                result = academia.download_pdf(pdf_url, output_path)
            ...
            results.append({
                ...
                'doi': doi,
                'status': 'not_found',
                'message': 'Paper not found on Academia.edu',
                'file_size': 0,
                'paper_url': None,
                'year': year,
                'timestamp': datetime.now().isoformat()
    ...
    results_df.to_csv(ACADEMIA_LOG, index=False)

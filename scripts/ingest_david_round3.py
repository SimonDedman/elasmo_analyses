#!/usr/bin/env python3
"""
Ingest David Ruiz Garcia's round 3 PDFs into the SharkPapers library.
...
Saves a CSV of all new papers (DOI, title, authors, year, new_filename)
to outputs/david_round3_new_papers.csv.

Usage:
    python scripts/ingest_david_round3.py
"""
...
OUTPUT_CSV = Path(
    "/media/simon/data/Documents/Si Work/PostDoc Work/EEA/2025/Data Panel"
    "/outputs/david_round3_new_papers.csv"
)
...
def count_authors_in_text(text: str) -> bool:
    """Heuristic: are there multiple authors?"""
    if not text:
        return False
    header = text[:1500]
    lines = [ln.strip() for ln in header.split('\n') if ln.strip()]
...
def process_pdfs() -> None:
    """Process all PDFs in David's folder."""
    # Collect PDFs
    ...
    print(f"Found {len(pdfs)} PDFs in David's folder")
    print()
    ...
    print(f"  {sum(len(v) for v in lib_index.values()):,} files indexed "
          f"across {len(lib_index):,} author+year keys")
    print()

    # Track results
    already_existed: list[tuple[str, str]] = []   # (original_name, match_info)
    newly_moved: list[dict] = []                   # full metadata dicts
    ...
    for i, pdf_path in enumerate(pdfs, 1):
        ...
        if doi:
            ...
            if nd in doi_lookup:
                ...
                # If found in parquet but not library, also check library
                ...
                if not found_in_library and parquet_rec:
                    ...
                    if p_surname_clean and p_year:
                        ...
                        if lib_matches2:
                            found_in_library = True
                            library_match_info = (
                                f"Library match via parquet ({key2}): "
                                f"{lib_matches2[0].name}"
                            )
                            print(f"  ALREADY IN LIBRARY (via parquet): "
                                  f"{library_match_info}")
        ...
        if found_in_library:
            ...
            if parquet_rec:
                ...
                final_authors_full = p_authors
                final_title_full = parquet_rec['title']
                try:
                ...
                final_authors_full = author or "Unknown"
                final_title_full = final_title
                final_year = year or "Unknown"
            ...
            if target.exists():
                try:
                    ...
                    already_existed.append(
                        (pdf_path.name, f"Exact filename match: {new_name}")
                    )
                    print(f"  -> DELETED (exact filename already exists: {new_name})")
                except Exception as e:
                    failed.append(
                        (pdf_path.name, f"Could not delete duplicate: {e}")
                    )
                continue
            ...
            try:
                ...
                newly_moved.append({
                    ...
                    'new_path': f"{final_year}/{new_name}",
                    'doi': doi,
                    'title': final_title_full,
                    'authors': final_authors_full,
                    'year': final_year,
                    'in_parquet': found_in_parquet,
                    'literature_id': parquet_rec['literature_id'] if parquet_rec else '',
                })
    ...
    if newly_moved:
        ...
        for rec in newly_moved:
            print(f"  {rec['original_name']}")
            print(f"    -> {rec['new_path']}")
            if rec['doi']:
                print(f"    DOI: {rec['doi']}")
            if rec['in_parquet']:
                print(f"    Already in parquet (lit_id={rec['literature_id']})")
            else:
                print(f"    NOT in parquet -- needs adding")
        print()
    ...
    # Save CSV of new papers
    if newly_moved:
        with open(OUTPUT_CSV, 'w', newline='', encoding='utf-8') as f:
            writer = csv.DictWriter(f, fieldnames=[
                'doi', 'title', 'authors', 'year', 'new_filename',
                'in_parquet', 'literature_id', 'original_name',
            ])
            writer.writeheader()
            for rec in newly_moved:
                writer.writerow({
                    ...
                    'in_parquet': rec['in_parquet'],
                    'literature_id': rec['literature_id'],
                    'original_name': rec['original_name'],
                })
        print(f"CSV saved: {OUTPUT_CSV}")
        print(f"  {len(newly_moved)} new papers recorded")
    else:
        print("No new papers to save to CSV.")
